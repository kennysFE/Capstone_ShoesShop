...
export default function Search() {
  const { listProduct, cartsort } = useSelector(
  ...
  const form = useFormik({
    ...
    onSubmit: (input) => {
      dispatch(searchProductApi(input.searchValue));
    },
  });
  ...
  function handleDebounceFunc(searchValue) {
    dispatch(searchProductApi(searchValue));
  }
  ...
  const renderProduct = (cartsort) => {
    // console.log("cartsort", cartsort);
    if (!cartsort || cartsort.length === 0) {
      return <h1 className="no-result">Không có kết quả</h1>;
    }
    return cartsort.map((product, index) => {
      return (
